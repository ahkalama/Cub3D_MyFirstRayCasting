...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/08 14:19:29 by ahkalama          #+#    #+#             */
/*   Updated: 2024/02/26 15:01:40 by ahkalama         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	t_coordinates	ft_y_point(t_coordinates pos, double angle)
{
	...
	if (angle > M_PI_2 && angle < 3 * M_PI_2)
		...
		tmp.x -= 0.00000000001;
	tmp.y = pos.y + ((sin(angle) / cos(angle)) * (tmp.x - pos.x));
	return (tmp);
}
...
static	t_coordinates	ft_x_point(t_coordinates pos, double angle)
{
	...
	if (angle > M_PI)
		...
		tmp.y -= 0.00000000001;
	tmp.x = pos.x + (tmp.y - pos.y) / (sin(angle) / cos(angle));
	return (tmp);
}
